...
def extract_edge_attributes(data, pos_enc_dim):
    ...
    if data.x is None:
        pe = add_laplacian_eigenvector_pe(data.edge_index, data.num_nodes, k=pos_enc_dim, is_undirected=True)
    else:
        pe = add_laplacian_eigenvector_pe(data.edge_index, data.x.shape[0], k=pos_enc_dim, is_undirected=True)
