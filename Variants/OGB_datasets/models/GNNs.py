...
class AtomEncoder(torch.nn.Module):
    ...
    def forward(self, x):
        x_embedding = 0
        for i, emb_layer in enumerate(self.atom_embedding_list):
            # Ensure the index is within the vocabulary size
            # Create a copy of the tensor to avoid inplace modification
            x_copy = x[:, i].clone()
            # Replace values >= emb_layer.num_embeddings with a constant value
            x_copy = torch.where(x_copy >= emb_layer.num_embeddings, torch.tensor(emb_layer.num_embeddings - 1), x_copy)
            x_embedding += emb_layer(x_copy)
...
class GNN(nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.atom_encoder(data.x.to(torch.long)) # .to(torch.float)
        
        # edge_attr_embedding = self.init_edge_attr_emb(data)
        edge_attr_embedding = self.bond_encoder(edge_attr)
        edge_attr_v2_embedding = self.init_edge_attr_v2_emb(data)
        # edge_attr_v2_embedding = edge_attr_v2
