...
# from .positional_encoding import add_laplacian_eigenvector_pe
...
class TUDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for itr, datum in enumerate(self.data):
            pe = add_laplacian_eigenvector_pe(datum.edge_index, datum.x.shape[0], itr, k=self.pos_enc_dim, is_undirected=True)
            datum.pos = torch.tensor(pe, dtype=torch.float32)
...
class TUDatasetGINSplit(InMemoryDataset):
    ...
    def __init__(self, name, root='tudatasets_gin', pos_enc_dim=6, transform=None, pre_transform=None):
        self.name = name
    ...
    def process(self):
        ...
        data_list = [Data(x=datum.node_features, edge_index=datum.edge_mat, y=torch.tensor(datum.label).unsqueeze(0).long()) for
            datum in data]
        
#         for itr, datum in enumerate(data_list):
#             pe = add_laplacian_eigenvector_pe(datum.edge_index, datum.x.shape[0], itr, k=self.pos_enc_dim, is_undirected=True)
#             datum.pos = torch.tensor(pe, dtype=torch.float32)
