...
def get_unique_random_value(data, used_indices):
    # Filter out the used indices and select randomly from the remaining
    remaining_indices = [i for i in range(len(data)) if i not in used_indices]
    if not remaining_indices:
        raise Exception("No more unique items left to select from.")
    index = torch.randint(len(remaining_indices), (1,)).item()
    new_index = remaining_indices[index]
    used_indices.append(new_index)
    return data[new_index]


# def set_random_seed(data, used_indices):
#     # Filter out the used indices and select randomly from the remaining
#     remaining_indices = [i for i in range(len(data)) if i not in used_indices]
#     if not remaining_indices:
#         raise Exception("No more unique items left to select from.")
