...
from torch_geometric.utils import dropout_adj, dropout_edge, to_networkx, to_undirected, degree, to_scipy_sparse_matrix, \
    from_scipy_sparse_matrix, sort_edge_index, add_self_loops, subgraph, k_hop_subgraph
from torch.distributions.bernoulli import Bernoulli
import pdb

def permute(x: torch.Tensor, pos: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    """
...
def dropout_feature(x: torch.FloatTensor, drop_prob: float) -> torch.FloatTensor:
    return F.dropout(x.to(torch.float), p=1. - drop_prob) # drop_feat.to(torch.long)
...
def compute_ppr(edge_index, edge_weight=None, edge_attr_v2=None, alpha=0.2, eps=0.1, ignore_edge_attr=True, add_self_loop=True):
    ...
    # pdb.set_trace()
    # new_edges = set(map(tuple, edge_index.t().tolist()))
    # edge_attr_v2 = edge_attr_v2[[i for i, edge in enumerate(original_edge_index.t().tolist()) if tuple(edge) in new_edges]]
    # edge_weight = original_edge_weight[[i for i, edge in enumerate(original_edge_index.t().tolist()) if tuple(edge) in new_edges]]

    return edge_index, edge_weight, edge_attr_v2
...
def drop_node(edge_index: torch.Tensor, edge_weight: Optional[torch.Tensor] = None, keep_prob: float = 0.5) -> (torch.Tensor, Optional[torch.Tensor]):
    ...
    edge_index, edge_weight, edge_mask = subgraph(subset, edge_index, edge_weight, return_edge_mask=True)
...
def random_walk_subgraph(edge_index: torch.LongTensor, edge_weight: Optional[torch.FloatTensor] = None, batch_size: int = 1000, length: int = 10):
    ...
    edge_index, edge_weight, edge_mask = subgraph(node_idx, edge_index, edge_weight, return_edge_mask=True)
