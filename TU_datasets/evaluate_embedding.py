...
def logistic_classify(x, y):
    ...
    return np.mean(accs_val), np.std(accs_val), np.mean(accs), np.std(accs)
...
def evaluate_embedding(embeddings, labels, search=True):
    ...
    _acc_val, _std_val, _acc, _std = svc_classify(x, y, search)
    if _acc_val > acc_val:
        acc_val, std_val = _acc_val, _std_val
        acc, std = _acc, _std

    return acc_val, std_val, acc, std